 ...
 * @param {headingPrefix} - a prefix for headings, zero, one or more # characters
 */
Schema.process = function(schemaMap, schemaPath, docDir, schemaDir, metaElements, readme, docs, headingPrefix) {
  smap=schemaMap;
  ...
  return Promise.mapSeries(keys, schemaKey => {
    ...
    return Schema.getExamples(schemaMap[schemaKey].filePath, schema)
      ...
      .then(allSchema => {
        ...
        const outputTasks = [ markdownWriter(schemaMap[schemaKey].filePath, object.mSchema, schemaPath, docDir, object.dep, docs, headingPrefix) ];
        if (schemaDir !== '') {
