...
const generateMarkdown = function(filename, schema, schemaPath, outDir, dependencyMap, docs, headingPrefix) {
  var ctx = {
    schema: schema,
    _: _,
    validUrl: validUrl,
  ...
  multi = multi.map(([ template, context ]) => {
    return [
      ...
      assoc(assoc(assoc(context, '_', _), 'simpletype', simpletype), 'headingPrefix', headingPrefix)
    ];
  });
